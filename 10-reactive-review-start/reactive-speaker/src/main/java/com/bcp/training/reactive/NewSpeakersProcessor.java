...
public class NewSpeakersProcessor {
    ...
    public CompletionStage<Void> sendEventNotifications(Message<SpeakerWasCreated> message) {
        ...
        if (event.affiliation == Affiliation.RED_HAT) {
            ...
            EmployeeSignedUp employeeEvent = new EmployeeSignedUp(
                    event.id,
                    event.fullName,
                    event.email
            );
            employeeEmitter.send(employeeEvent);
        } else if (event.affiliation == Affiliation.GNOME_FOUNDATION) {
            logEmitEvent("UpstreamMemberSignedUp", event.affiliation);

            UpstreamMemberSignedUp upstreamEvent = new UpstreamMemberSignedUp(
                    event.id,
                    event.fullName,
                    event.email
            );
            upstreamEmitter.send(upstreamEvent);
        }
        return message.ack();
    }
